 ...
 *       Se debe ejecutar antes de usar cualquier otra funci?n!!!
 * 
 ...
 * 
 * - void LCD_Blink(void)
 *       Si el cursor se encuentra visible, este se visualizar? como un cuadro
 *       parpadeando.
 * 
 * - void LCD_noBlink(void)
 *       Si el cursor se encuentra visible, este se visualizar? como una linea
 *       estatica.
 * 
 ...
 *       Cada vez que se escriba un caracter, el cursor se desplazar? a la
 *       izquierda.
 * 
 ...
 *       La pantalla se desplazar? automaticamente al escribir un nuevo
 *       caracter.
 * 
 * - void LCD_noAutoscroll(void)
 *       La pantalla permanecer? estatica en su posicion actual.
 * 
 ...
 *       Escribe un n?mero entero en la fila y columna (row, col).
 *       -> int num = -120;
 ...
 *
 * - Si el LCD est? en modo 8-bits descomentar #define LCD_USE_8BITS_DATA
...
//#define LCD_RS_DIR  TRISBbits.TRISB3         
//
...
#define LCD_D4_IN   PORTAbits.RA4
#define LCD_D5_IN   PORTAbits.RA5
#define LCD_D6_IN   PORTAbits.RA6
#define LCD_D7_IN   PORTAbits.RA7

#define LCD_D4_OUT  LATAbits.LATA4
#define LCD_D5_OUT  LATAbits.LATA5
#define LCD_D6_OUT  LATAbits.LATA6
#define LCD_D7_OUT  LATAbits.LATA7

#define LCD_D4_DIR  TRISAbits.TRISA4
#define LCD_D5_DIR  TRISAbits.TRISA5
#define LCD_D6_DIR  TRISAbits.TRISA6
#define LCD_D7_DIR  TRISAbits.TRISA7
...
#else
#define LCD_DIR_IN()    TRISA |= 0xF0
#define LCD_DIR_OUT()   TRISA &= 0x0F
...
//*************** CONFIGURACI?N DE TIEMPOS/RETARDOS/INTENTOS *******************
...
/*
 * Funci?n LCD_Init()
 *  Permite inicializar el LCD y la libreria para su correcta ejecuci?n.
 *  Esta funci?n se debe ejecutar al principio y antes de utilizar las demas
...
/*
 * Funci?n LCD_Write()
 *  Permite enviar la informacion al LCD seg?n la configuracion establecida
 *  [8-bits/4-bits].
 *  Antes de ejecutarse esta funci?n, se debe establecer RS si la informacion es
 *  dato o comando.
 *
 *  value: Dato o comando a enviar.
 *  mode:  Para modo 4-bits, si es 1, solo env?a el nibble alto, y si es 0,
 *         env?a los 2 nibbles.
 *
 */
...
/*
 * Funci?n LCD_Text_pos()
 *  Visualiza en el LCD una cadena de caracteres o texto en determinada posici?n
 *  seg?n (row,col).
 *
 *  row:  Fila
 *  col:  Columna
...
/*
 * 
 * Funci?n LCD_CustomChar()
 *  Permite almacenar en la memoria CGRAM del LCD, un caracter personalizado.
 *  Antes de ejecutar esta funci?n, se debe ejecutar LCD_CustomMem(...) para
...
/*
 * Funci?n LCD_PrintNumber()
 *  Permite visualizar en el LCD un n?mero entero [+/-].
 *  
 *  num: N?mero entero a visualizar en el LCD.
 *  
 */

#define LCD_PrintNumber_pos(row,col,num) LCD_SetCursor(row,col);LCD_PrintNumber(num)
/*
 * Funci?n LCD_PrintNumber_pos()
 *  Permite visualizar en el LCD un n?mero entero [+/-] en determinada posici?n
 *  seg?n (row,col).
 *  row:  Fila
 *  col:  Columna
 *  num:  N?mero entero a visualizar en el LCD.
...
/*
 * Funci?n LCD_Busy()
 *  Si RW esta configurado a un pin del uC, esta funci?n permite la comprobaci?n
 *  de si el LCD se encuentra disponible para recibir una nueva instrucci?n o
 *  comando.
 *  Si RW no est? configurado, realiza un retardo para esperar a que el LCD se
 *  encuentre desocupado.
 */

#define LCD_Write_Nibble(dato)  LCD_RS = LCD_CMD; LCD_Write(dato,0)
/*
 * Funci?n LCD_Write_Nibble()
 ...
 *  se encuentra habilitado su visualizaci?n.
 *  
 */
 ...
 *  que se encuentra hilitado su visualizaci?n.
 *  
 */

#define LCD_scrollDisplayLeft()  LCD_Command(DISPLAY_SHIFT_L)
/*
 * Funci?n LCD_scrollDisplayLeft()
...
/*
 *  Funci?n LCD_scrollDisplayRight()
 *  Permite desplazar la pantalla a la derecha para mostrar los caracteres
 *  ocultos en memoria.
 *  
 */
...
/*
 * Funci?n LCD_Autoscroll()
 *  Permite configurar al LCD para que la pantalla se desplace automaticamente
 *  al escribir un nuevo caracter. El cursor permanecer? est?tico en su posicion
 *  actual.
 *  
 */
...
/*
 * Funci?n LCD_noAutoscroll()
 *  Permite configurar al LCD para que la pantalla no se desplace
 *  automaticamente al escribir un nuevo caracter. El cursor se desplazar?
 *  dependiendo de la configuraci?n previa.
 *  
 */
 ...
 *  
 *  Por ejemplo, se desea especificar al LCD que se ingresar? un caracter
