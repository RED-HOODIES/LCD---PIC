...
void LCD_Init(void) {
    ...
    LATAbits.LATA3 = 0; LCD_Write(0x30,0);
    ...
    _delay((unsigned long)((5)*(1000000/4000.0)));
# 688 "./LCD_16x2.h"
    LATAbits.LATA3 = 0; LCD_Write(0x20,0);
    while (LCD_Busy()) {
        ;
    }
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x28,1);


    _displaycontrol = 0x04 | 0x00 | 0x00;
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);

    _displaymode = 0x02 | 0x00;
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x04 | _displaymode,1);
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x01,1);
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x02,1);
}
...
void LCD_Write(char value, char mode) {
    ...
    LATAbits.LATA7 = LCD_data.bit7;
    LATAbits.LATA6 = LCD_data.bit6;
    LATAbits.LATA5 = LCD_data.bit5;
    LATAbits.LATA4 = LCD_data.bit4;

    LATAbits.LATA2 = 1; _delay((unsigned long)((1)*(1000000/4000000.0))); LATAbits.LATA2 = 0;;
    if (mode) {

        LATAbits.LATA7 = LCD_data.bit3;
        LATAbits.LATA6 = LCD_data.bit2;
        LATAbits.LATA5 = LCD_data.bit1;
        LATAbits.LATA4 = LCD_data.bit0;
        LATAbits.LATA2 = 1; _delay((unsigned long)((1)*(1000000/4000000.0))); LATAbits.LATA2 = 0;;
...
void LCD_Text(void *pt) {
    ...
    while (*text) {
        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(*text,1);
        text++;
    }
}
...
void LCD_CustomChar(void *pt) {
    unsigned char *c = pt;
    for (char i = 0; i < 8; i++) {
        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(*c,1);
        c++;
    }
}
...
void LCD_print_number(unsigned long num, char sign) {
    if (num == 0) {
        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write('0',1);
        return;
    }
    if (sign) {
        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write('-',1);
        num = ~num + 1;
    }
    ...
    while (lcd_div) {
        char c = num / lcd_div;
        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write('0' + c,1);
        num %= lcd_div;
        lcd_div /= 10;
    }
}
...
void main(void) {
    ...
    LCD_Init();
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x40 | ((8 & 0x7) << 3),1); LCD_CustomChar(fig);

   while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x40 | ((1 & 0x7) << 3),1); LCD_CustomChar(C1);
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x40 | ((2 & 0x7) << 3),1); LCD_CustomChar(C2);
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x40 | ((3 & 0x7) << 3),1); LCD_CustomChar(C3);
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x80 + (0x40 * 0) + 0,1);



    LCD_Text("Testeo....");
    while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(8,1);




    _displaycontrol |= 0x02; while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
    _displaycontrol |= 0x01; while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
    char i, j;
    char valid;
    while (1) {

        for (j = 0; j < 4; j++) {
            while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x80 + (0x40 * 1) + 0,1);
            _displaycontrol |= 0x01; while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
            _delay((unsigned long)((1500)*(1000000/4000.0)));
            _displaymode |= 0x02;; while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x04 | _displaymode,1);
            i = 0;
            _displaycontrol &= ~0x01; while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write((unsigned)0x08 | _displaycontrol,1);
            do {
                switch (j) {
                    case 0:
                        ...
                        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(msg1[i],1);
                        break;
                    case 1:
                        valid = i < (sizeof (msg2) - 2);
                        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(msg2[i],1);
                        break;
                    case 2:
                        valid = i < (sizeof (msg3) - 1);
                        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(msg3[i],1);
                        break;
                    default:
                        valid = i < (sizeof (msg4) - 1);
                        while(LCD_Busy()); LATAbits.LATA3 = 1; LCD_Write(msg4[i],1);
                        break;
                }
                i++;
